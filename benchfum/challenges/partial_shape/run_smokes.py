"""Run all partial-shape smoke benchmarks.

Usage
-----
python -m benchfum.challenges.partial_shape.run_smokes
python -m benchfum.challenges.partial_shape.run_smokes --seed 7
python -m benchfum.challenges.partial_shape.run_smokes --stop-on-fail
"""
...
def _default_datasets_root() -> Path:
    # .../geomfum/benchfum/challenges/partial_shape/run_smokes.py
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run all partial-shape smoke benchmarks"
    )
    parser.add_argument(
        "--datasets-root",
        type=str,
    ...
    jobs = [
        (
            ...
            config_root / "shrec16_partial_smoke.json",
        ),
        (
            "CP2P24 smoke",
            datasets_root / "CP2P24" / "test",
            config_root / "cp2p_partial_smoke.json",
        ),
        (
            "PARTIALSMAL smoke",
            datasets_root / "PARTIALSMAL",
            config_root / "partialsmal_partial_smoke.json",
        ),
    ]
    ...
    print("\n=== Summary ===")
    if not failures:
        print("All partial smoke benchmarks passed.")
        return 0
